...
func main() {
	...
	if len(os.Args) < 6 {
		log.Println("Usage: HLSVODGenerator [generation_info_file_name] [server_ip] [the_number_of_session] [session_generation_interval] [play_time] [ server_port ] ")
		return
	}
	...
	for i := 0; i < SessionCount; i++ {
		...
		go func(u *url.URL, t int, n int) {
			...
			start := time.Now()
			content, _, err := getContent(u, client)
			if err != nil {
				log.Println("error:", err)
				return
			}
